...
export const addAvailability = async (userId, date, startTime, endTime) => {
  ...
  if (!staff) throw new ApiError(403, "لازم تكون حلاق لتحديد دوامك");
  ...
  if (existing) throw new ApiError(409, "عندك دوام مسجل أصلاً بهاد التاريخ");

  return await prisma.availability.create({
    data: { date: parsedDate, dayOfWeek, startTime, endTime, staffId: staff.id },
  });
};
...
export const getMyAvailability = async (userId) => {
  const staff = await prisma.staff.findUnique({ where: { userId } });
  if (!staff) throw new ApiError(403, "لازم تكون حلاق لعرض دوامك");
